 ...
 * @author sonam
 */
@WebServlet(name = "uploadCoverPic", urlPatterns = {"/uploadCoverPic"})
public class UploadCoverPicServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            List<FileItem> coverPic = sf.parseRequest(request);

            HttpSession session = request.getSession();
            String uname = (String) session.getAttribute("username");

            for (FileItem i : coverPic) {
